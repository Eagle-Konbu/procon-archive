...
int main()
{
    ...
    rep(i, q) cin >> x[i];
    rep(i, q) x[i]--;
    rep(i, q) edges[x[i]].second = false;

    for (auto edge : edges)
        if (edge.second)
            uf.merge(min(n, edge.first.first), min(n, edge.first.second));
    ...
    {
        ans[i] = uf.size(n) - 1;
        ll u = edges[x[i]].first.first, v = edges[x[i]].first.second;
        uf.merge(min(n, u), min(n, v));
    }
