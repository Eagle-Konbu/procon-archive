...
int main()
{
    ...
    vector turn_cnt_to_satisfy(n, vector<ll>(3, -1));

    rep(i, n)
    {
        turn_cnt_to_satisfy[p[i]][0] = (p[i] - i - 1) % n;
        turn_cnt_to_satisfy[p[i]][1] = (p[i] - i) % n;
        turn_cnt_to_satisfy[p[i]][2] = (p[i] - i + 1) % n;
    }
    ...
    rep(i, n)
    {
        ...
        if (minus_flg)
        {
            rep(j, 3)
            {
                turn_cnt_to_satisfy[i][j] += n;
                turn_cnt_to_satisfy[i][j] %= n;
            }
        }
    }

    vector<ll> turn_cnts(n);

    rep(i, n) rep(j, 3)
    {
        turn_cnts[turn_cnt_to_satisfy[i][j]]++;
    }

    cout << *max_element(all(turn_cnts)) << endl;
