...
int main()
{
    ...
    vector<vector<P>> prime_factors;
    rep(i, n)
    {
        prime_factors.push_back(prime_factor(a[i]));
    }
    ...
    ll min_p = LLONG_MAX, min_q = LLONG_MAX;
    ...
    rep(i, n)
    {
        for (auto p : prime_factors[i])
        {
            if (p.first == 2)
            {
                chmin(min_p, p.second);
            }
            else if (p.first == 3)
            {
                chmin(min_q, p.second);
            }
        }
    }
