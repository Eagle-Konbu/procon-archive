...
int main()
{
    ...
    vector<ll> sizes(n * 2 + 1);
    sizes[0] = 0;
    reps(i, n * 2) sizes[i] = sizes[i - 1] + a[(i - 1) % n];

    ll right = 0;
    rep(left, n * 2)
    {
        while (right < n * 2 && sizes[right + 1 + 1] - sizes[left] <= size / 10)
        {
            right++;
        }
        if ((sizes[right + 1] - sizes[left] == size / 10) && (right - left < n))
        {
            cout << "Yes" << endl;
            return 0;
        }
