...
int main()
{
    ...
    reps(i, n) reps(j, s)
    {
        if (j - card[i - 1].first >= 0)
            dp[i][j] = dp[i - 1][j - card[i - 1].first];
        if (j - card[i - 1].second >= 0)
            dp[i][j] = dp[i][j] || dp[i - 1][j - card[i - 1].second];
    }
    ...
    rreps(i, n)
    {
        if (s_tmp - card[i - 1].first >= 0 && dp[i - 1][s_tmp - card[i - 1].first])
        {
            ans += "H";
            s_tmp -= card[i - 1].first;
        }
        else
        {
            ans += "T";
            s_tmp -= card[i - 1].second;
        }
    }
    reverse(all(ans));
    cout << ans << endl;
