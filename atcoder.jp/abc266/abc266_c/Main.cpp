...
int main()
{
    ...
    rep(i, 4)
    {
        P ab = {points[i].first - points[(i + 1) % 4].first, points[i].second - points[(i + 1) % 4].second};
        P bc = {points[(i + 1) % 4].first - points[(i + 2) % 4].first, points[(i + 1) % 4].second - points[(i + 2) % 4].second};
