...
bool solve(ll x, vector<ll> &a, bool is_x = false)
{
    ll n = a.size(), delta = 10004;
    vector dp(n + 1, vector<bool>(delta * 2 + 1, false));

    dp[0][0 + delta] = true;
    reps(i, n)
    {
        for (ll j = -delta; j <= delta; j++)
        {
            ll k = j + delta;
            if (k - a[i - 1] >= 0)
                dp[i][k] = dp[i][k] || dp[i - 1][k - a[i - 1]];
            if (k + a[i] <= 2 * delta)
                if (!is_x || i > 1)
                    dp[i][k] = dp[i][k] || dp[i - 1][k + a[i - 1]];
        }
...
int main()
{
    ...
    vector<ll> points_x, points_y;
    reps(i, n)
    {
        if (i % 2 == 1)
            points_x.push_back(a[i]);
        else
            points_y.push_back(a[i]);
    }

    if (solve(x, points_x, true) && solve(y, points_y))
        cout << "Yes" << endl;
    else
        cout << "No" << endl;
