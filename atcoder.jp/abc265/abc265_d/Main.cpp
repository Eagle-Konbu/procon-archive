...
int main()
{
    ...
    vector<ll> sum_list(n, 0);
    sum_list[0] = a[0];
    reps(i, n - 1) sum_list[i] = sum_list[i - 1] + a[i];

    ll sum1 = 0, right = 0;
    rep(left, n)
    {
        while (right < n && sum1 < p)
        {
            sum1 += a[right];
            right++;
        }
        if (sum1 == p)
        {
            ll target = p + q;
            if (left > 0)
                target += sum_list[left - 1];
            ll y = right, z = POSL(sum_list, target) + 1;
            if (sum_list[z - 1] == target)
            {
                ll w = POSL(sum_list, target + r) + 1;
                if (sum_list[w-1] == target + r)
                {
                    cout << "Yes" << endl;
                    return 0;
                }
            }
            // ll y = right, sum_4_q = 0;
            // for (ll i = y; i < n; i++)
            // {
            //     sum_4_q += a[i];
            //     if (sum_4_q == q)
