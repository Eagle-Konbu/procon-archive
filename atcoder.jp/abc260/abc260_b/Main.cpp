...
int main()
{
    ...
    vector<P> english(n);
    rep(i, n) english[i] = {-b[i], i};
    vector<P> total(n);
    rep(i, n) total[i] = {-(a[i] + b[i]), i};
    ...
    while (cnt < y)
    {
        if (!passed[english[idx].second])
        {
            cnt++;
            passed[english[idx].second] = true;
        }
        idx++;
    }
    cnt = 0;
    ...
    while (cnt < z)
    {
        if (!passed[total[idx].second])
        {
            cnt++;
            passed[total[idx].second] = true;
        }
        idx++;
    }
