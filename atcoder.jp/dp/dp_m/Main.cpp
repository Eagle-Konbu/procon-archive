...
int main()
{
    ...
    vector<fenwick_tree<mint>> bit_list(n + 1);

    vector dp(n + 1, vector<mint>(k + 1));
    rep(i, n + 1) dp[i][0] = 1;

    // rep(i, k) bit_list[0].add(i, dp[0][i]);

    reps(i, n)
    {
        bit_list[i - 1] = fenwick_tree<mint>(k + 1);
        rep(j, k + 1) bit_list[i - 1].add(j, dp[i - 1][j]);

        reps(j, k)
        {
            dp[i][j] = bit_list[i - 1].sum(max(0LL, j - a[i]), j + 1);
        }
    }
