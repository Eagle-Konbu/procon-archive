...
void dfs(const Graph &g, ll v, vector<bool> &used, vector<ll> &cnt, vector<ll> &c, vector<ll> &res)
{
    used[v] = true;
    if(cnt[c[v]] == 1)
    {
...
int main()
{
    ...
    vector<ll> cnts(100005);
    cnts[c[0]]++;

    vector<bool> used(n);
    vector<ll> ans;
    dfs(g, 0, used, cnts, c, ans);
